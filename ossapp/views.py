...
def index(request):
    cate=Category.get_all_category()
    products= Product.get_products_by_categoryid(10)
...
def searchMatch(query,item):
    if query in item.desc or query in item.name or item.category:
        return True
    else:
        return False
def search(request):
    ...
    for cat in categ:
        ...
        for item in prodtemp :
            if searchMatch(query,item):
                 print("hello")
    param={'products':data,'category':categ}
...
def payment(request):
    ...
    return render(request,'rez_order.html',{'price':price})